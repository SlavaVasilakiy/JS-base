...
const positive = Number(prompt("Введите положительное значение:")); // тут я что-то эксперементировал и забыл поменять на const обратно
const negative = Number(prompt("Введите отрицательное значение:"));

if ((positive > 0) && (negative < 0)) { // форматировщик сам оборачивает в скобки. && - проглядел
  alert("Все значения верные.");
} else {
  alert(`Одно или более значений некорректно.
Вы ввели: ${positive} и ${negative}`);
}
